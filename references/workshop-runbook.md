...
1. Start with `--mode discover` and a page-only Scope before opening the approved Workshop page.
2. Review `observed-hosts.json`, obtain approval for each static CDN, then write the strict capture Scope.
3. Start strict capture before the approved page load. Mark `P0:baseline`, open the empty page visibly, and wait until no new static-resource requests appear for 3–5 seconds.
4. Open one batch and mark its page mount.
5. Before each component state, enter a marker such as `mark P2:ObjectTable:filter-open`.
6. Trigger only normal UI states: activate tabs, expand accordions, open dialogs, expose virtualized/viewport content, and wait for data-backed rendering.
7. Wait for 3–5 seconds of static-resource quiet before the next marker. Use fixed, operator-reviewable pacing; do not add random jitter.
8. End after 8–10 components, inspect `risk-events.ndjson`, `invalid-assets.ndjson`, and the configured budget, then continue only within the approved window.
9. Run `audit-capture.mjs` after stopping. Run at most one warm-cache verification pass when required.
