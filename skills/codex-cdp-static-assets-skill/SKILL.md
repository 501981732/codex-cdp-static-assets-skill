...
3. Confirm case ID, exact page host and Module, test account, time/traffic limits, stop contact, asset types, autosave permission, publish permission, capture-page permission, and whether `allowModuleVariables` is authorized. Exact synthetic fixture mappings remain optional overrides.
...
Present one consolidated Scope containing exact host candidates and all intended actions: reload the exact page, open Add Widget, scroll the visible catalog/canvas, add widgets, create deterministic capture pages when authorized, automatically handle Module variables when `allowModuleVariables: true`, save and publish when `allowPublish: true`, enter development preview, optionally capture element-level state screenshots, and read completed approved response bodies.
...
4. Open Add Widget with visible UI. Repeatedly use `take_snapshot` and visible scroll/End operations until the catalog tracker reports two consecutive bottom observations with no new canonical widget key. Build the key from visible label, adding category and version/type only when shown. Cross-snapshot repeats are deduplicated; record indistinguishable duplicate keys as `catalog-identity-ambiguity`, skip only those entries, and continue the remaining catalog.
...
8. Exercise the three-state matrix with visible tools and a stable marker, in this fixed order: add → configure variable input when present → save and publish → development preview → resource capture.
   ...
   - `data-bound`: only for Widgets with a visible variable-input capability. When `allowModuleVariables: true`, create, select, modify, or clean up variables through visible UI as needed for the Widget. If the visible UI offers no compatible variable path, record `not-requested` for an optional input or `blocked-missing-fixture` for a required input, then continue. Opening configuration is an action within this state, not a separate coverage state.
   - Save the Module through approved autosave. When `allowPublish: true`, use the visible publish control and verify completion before preview; otherwise record `blocked-missing-publish-authority` and do not enter preview.
   - `preview-visible`: enter visible development preview and scroll the Widget into view; this remains separate because preview can load a distinct runtime bundle. Only after preview, read the completed approved responses naturally observed since Widget addition; retain their state markers so add/config/preview loads remain attributable.
...
10. At the baseline, after Catalog opening, and after an action that can load a resource (add, successful data binding, or preview), inspect the complete request list first. Ignore `chrome-extension://` requests as local browser noise: never read, save, or authorize them. Stop before body access on an unknown `http://` or `https://` host, `401`, `403`, `429`, repeated `5xx`, CAPTCHA/MFA, logout, account warning, or unexpected write. When new request IDs or statuses appear, wait for baseline plus two unchanged observations; otherwise continue without a redundant stability loop.
...
Never use `evaluate_script`. Never set `requestFilePath`; `get_network_request` may receive only a response output path under the resolved `os.tmpdir()` staging root. Never clear/disable cache, bypass Service Workers, intercept requests, replay URLs, guess/probe additional chunks, probe sourcemaps, expose hidden routes, change permissions, execute actions/workflows, export data, or inspect request payloads. Module variable handling is allowed only through visible UI when `allowModuleVariables: true`; publishing is allowed only through visible UI when `allowPublish: true`. The baseline inventory may record Type IDs, Renderer names, Chunk IDs, and module IDs literally present in already retained JS, but must never retrieve those referenced chunks.
