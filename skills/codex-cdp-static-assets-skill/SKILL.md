...
3. Confirm case ID, exact page host and Module, test account, time/traffic limits, stop contact, asset types, autosave permission, capture-page permission, and the explicit permissions to use, create, modify, or delete variables in the approved Module. Exact synthetic fixture mappings remain optional overrides.
...
Present one consolidated Scope containing exact host candidates and all intended actions: reload the exact page, open Add Widget, scroll the visible catalog/canvas, add widgets, create deterministic capture pages when authorized, create, modify, or delete Module variables only when each corresponding `allow*ModuleVariables` permission is authorized, open configuration, accept autosave, enter preview, optionally capture element-level state screenshots, and read completed approved response bodies.
...
8. Exercise the three-state matrix with visible tools and a stable marker:
   ...
   - `data-bound`: only for Widgets with a visible data-input capability. Use the visible Module variable controls within the approved permissions: create a mapped `CDP Capture` test variable only when `allowCreateTestVariables: true`; modify a compatible variable only when `allowModifyModuleVariables: true`; select existing variables only when `allowExistingModuleVariables: true`; and delete a test variable during cleanup only when `allowDeleteModuleVariables: true`. If no permitted compatible variable path exists, record `not-requested` for an optional source or `blocked-missing-fixture` for a required source, then continue. Never create, modify, or delete a data source. Opening configuration is an action within this state, not a separate coverage state. Allow only approved autosave.
...
Never use `evaluate_script`. Never set `requestFilePath`; `get_network_request` may receive only a response output path under the resolved `os.tmpdir()` staging root. Never clear/disable cache, bypass Service Workers, intercept requests, replay URLs, guess/probe additional chunks, probe sourcemaps, expose hidden routes, change permissions, publish, execute actions/workflows, export data, or create/modify/delete a data source. Module variable mutations are allowed only through visible UI and only for the matching explicit Scope permission. The baseline inventory may record Type IDs, Renderer names, Chunk IDs, and module IDs literally present in already retained JS, but must never retrieve those referenced chunks.
