...
需要 Node.js 22 或更高版本，以及支持 loopback CDP 的 Chrome/Chromium。优先复用已经登录且已启用 CDP 的可见 Chrome；Skill 不会搬运 Cookie 或登录凭证。
...
| Chrome | 优先复用已登录、已启用 loopback CDP 的可见会话；同一授权主机只保留一个目标页 |
...
你先在已启用 CDP 的可见 Chrome 中正常登录并打开空白 Workshop，然后回复“已登录并打开空白 Workshop”。Codex 会只附着到唯一匹配授权主机的页面；其他普通标签页可以保持打开，不会被采集或写入日志。Discovery 启动后，你在可见页面中手动刷新并做一次代表性的基础导航，然后回复：
...
默认复用当前已批准的 CDP Chrome 会话及其原地登录态完成 Discovery 和严格采集，不要求仅为采集另开一个 Chrome。已经进入浏览器缓存的资源可能显示为 `body-unavailable` 或空 `304`；记录缺口即可，不主动补请求。

如果当前 Chrome 没有 loopback CDP，采集器无法在进程启动后临时补开它。Chrome 136 以后也不会对默认用户目录接受命令行远程调试参数。此时暂停，由你选择已登录的持久 Capture Profile 或其他已批准的 CDP 连接；Skill 不自动新开第二个 Profile。

如果授权方确实要求更完整的响应正文，可以单独批准新的 Capture Profile，仅执行严格基线。无论哪种方案，都不清缓存、不禁用缓存、不绕 Service Worker、不复制或转移 Cookie、Token、密码及 Profile 文件，也不主动重放资源 URL。
